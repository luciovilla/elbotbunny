...
export default function DropDown({ song, setSong }: DropDownProps) {
  ...
  return (
    <Combobox value={song} onChange={setSong}>
            ...
            {filteredSongs.length === 0 && query !== "" ? (
              ...
              filteredSongs.map((item) => (
                <Combobox.Option
                  key={item.title}
                  className={({ active }) =>
                    `relative cursor-default select-none py-2 pl-10 pr-4 h-8 ${
